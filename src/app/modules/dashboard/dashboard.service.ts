...
import { AboutUs, Adds, ContactSupport, Faq, Helps, PrivacyPolicy, Recipe, Review, Subscribe, Subscription, TermsConditions } from "./dashboard.model";
...
const getAllRecipes = async (user: IReqUser, query: any, payload: any) => {
    ...
    if (query?.ratting) {
        filterQuery.ratting = { $lte: Number(query.ratting) };
    }
...
const getRecipeDetails = async (id: Types.ObjectId, userId: string) => {
    ...
    //@ts-ignore
    result.favorite = result.favorites.some((id) => id.toString() === userId.toString());
...
const addHelps = async (payload: any) => {
    const checkIsExist = await Helps.findOne();
    if (checkIsExist) {
        return await Helps.findOneAndUpdate({}, payload, {
            new: true,

            runValidators: true,
        });
    } else {
        return await Helps.create(payload);
    }
};
const getHelps = async () => {
    return await Helps.findOne();
};

const sendSubscribe = async (payload: any) => {

    const data = await Subscribe.create(payload);
    if (!data) {
        throw new ApiError(400, "Error creating subscribe")
    }
    return data
};
const getSubscribe = async (query: any) => {
    const { month, page = 1, limit = 10 } = query;

    const filter: any = {};

    if (month) {
        const [mm, yyyy] = (month as string).split("-");
        if (!mm || !yyyy) {
            throw new Error("Invalid month format, use MM-YYYY");
        }

        const startDate = new Date(Number(yyyy), Number(mm) - 1, 1);
        const endDate = new Date(Number(yyyy), Number(mm), 0, 23, 59, 59, 999);
    ...
    const [data, total] = await Promise.all([
        Subscribe.find(filter).skip(skip).limit(Number(limit)).sort({ createdAt: -1 }),
...
const getRecipesReviewsW = async (recipeId: string, query: {
    page?: number;
    limit?: number;
}) => {
    ...
    const results = await addsQuery.modelQuery;
    const meta = await addsQuery.countTotal();
